...
const unsigned long periodESP = 600000;  // 10 menit timeout (production mode)
...
const unsigned long writeStateDelay = 500;  // 500ms delay antar penulisan file (production mode)
...
const unsigned long periodPZEM = 600000; // 10 menit (production mode)

//Timer untuk INA219
unsigned long startMillisINA;
unsigned long currentMillisINA;
const unsigned long periodINA = 600000; // 10 menit (sinkron dengan PZEM)

//Timer untuk ATS
unsigned long startMillisATS;
unsigned long currentMillisATS;
const unsigned long periodATS = 600000; // 10 menit (sinkron dengan PZEM)

//Timer untuk SD CARD (PRODUCTION: 10 menit per cycle)
unsigned long startMillisSOC;
unsigned long currentMillisSOC;
const unsigned long periodSOC = 600000;  // 10 menit untuk production mode
...
void setup() {
  ...
  // Inisialisasi timer dengan staggered start untuk robustness
  unsigned long baseTime = millis();
  startMillisPZEM = baseTime;
  startMillisSOC = baseTime + 30000;    // Offset 30 detik
  startMillisESP = baseTime + 60000;    // Offset 1 menit 
  ...
  Serial.println("  - SOC: 30s offset");
  Serial.println("  - ESP32: 60s offset");
  Serial.println("  - INA219: 90s offset");
  Serial.println("  - ATS: 120s offset");
  Serial.println("  - SD Check: 15s offset");
  ...
  Serial.println("🚀 STM32 PRODUCTION MODE");
  Serial.println("📊 Data Collection Intervals:");
  Serial.println("   - PZEM/INA219/ATS: 10 minutes");
  Serial.println("   - SOC Calculation: 10 minutes");  
  Serial.println("   - ESP32 Timeout: 10 minutes");
  Serial.println("   - SD Card Check: 30 seconds");
  Serial.println("   - TimeRef Update: 5 minutes");
...
unsigned long getCurrentTimestamp() {
    ...
    if (currentMillis < globalTimeRefBaseMillis) {
        ...
        Serial.println("⚠️ [TIMEREF] Millis overflow detected, recalibrating...");
...
void receiveESP32Data() {
    if (Serial3.available()) {
        ...
        // Format baru: timestamp,lux,temp1,temp2
        int commaIndex1 = receivedData.indexOf(',');
        int commaIndex2 = receivedData.indexOf(',', commaIndex1 + 1);
        int commaIndex3 = receivedData.lastIndexOf(',');
        
        if (commaIndex1 > 0 && commaIndex2 > commaIndex1 && commaIndex3 > commaIndex2) {
            espTimestamp = receivedData.substring(0, commaIndex1).toInt();
            espLux = receivedData.substring(commaIndex1 + 1, commaIndex2).toFloat();
            espTemp1 = receivedData.substring(commaIndex2 + 1, commaIndex3).toFloat();
            espTemp2 = receivedData.substring(commaIndex3 + 1).toFloat();
            ...
            if (espTimestamp > 1000000 && espTimestamp > globalTimeRef) {
                // Timestamp valid (> 1000000 = sekitar tahun 1970+)
                // dan lebih baru dari timeref yang ada
            ...
            int oldComma2 = receivedData.lastIndexOf(',');
            
            if (oldComma1 > 0 && oldComma2 > oldComma1) {
                espTimestamp = millis() / 1000; // Use system time as fallback
                espLux = receivedData.substring(0, oldComma1).toFloat();
                espTemp1 = receivedData.substring(oldComma1 + 1, oldComma2).toFloat();
                espTemp2 = receivedData.substring(oldComma2 + 1).toFloat();
...
void prepareCompleteData() {
    ...
    newData.timestamp = getCurrentTimestamp();  // Gunakan timeref global
    ...
    Serial.print(" (from ");
    Serial.print(timeRefAvailable ? "TimeRef" : "Fallback");
    Serial.print(") | SOCi2 V_pv: ");
...
void loop() {
    unsigned long currentTime = millis();
    currentMillisPZEM = currentTime;
    currentMillisSOC = currentTime;
    currentMillisINA = currentTime;
    currentMillisATS = currentTime;
    currentMillisSDCheck = currentTime;

    // ===== TIMER OVERFLOW PROTECTION =====
    // Deteksi millis() overflow (terjadi setiap ~49 hari)
    static unsigned long lastMillis = 0;
    if (currentTime < lastMillis) {
        Serial.println("⚠️ [TIMER] millis() overflow detected! Resetting all timers...");
        startMillisPZEM = currentTime;
        startMillisSOC = currentTime + 30000;
        startMillisESP = currentTime + 60000;
        startMillisINA = currentTime + 90000;
        startMillisATS = currentTime + 120000;
        startMillisSDCheck = currentTime + 15000;
        lastTimeRefUpdate = currentTime;
        writeStateTimer = currentTime;
        Serial.println("✅ [TIMER] All timers reset for overflow protection");
    }
    ...
    if (Serial.available()) {
        ...
        if (command == "sd" || command == "sdtest") {
            ...
            if (SD.exists("/timeref.txt")) {
                ...
                Serial.println("🗑️ [TIMEREF] Removed timeref.txt file");
            }
            digitalWrite(SPI1_NSS_PIN, HIGH);
