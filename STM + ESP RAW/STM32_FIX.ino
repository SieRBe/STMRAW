...
void ATS() {
  ...
  if (espSOC < 30) {
    ...
    isPLTS = false;
    waitToTurnOn = false;
    Serial.println("🔋 SOC < 30% (ESP32) - Relay OFF (PLN)");
    return;
  }
  ...
  if (espSOC >= 90 && !isPLTS) {
    if (!waitToTurnOn) {
      waitToTurnOn = true;
      timeToTurnOn = now;
      Serial.println("⏳ SOC ≥ 90% (ESP32) - Tunggu 5 detik sebelum Relay ON");
    }
    ...
    if (now - timeToTurnOn >= 5000) {
      ...
      isPLTS = true;
      waitToTurnOn = false;
      Serial.println(" SOC ≥ 90% stabil (ESP32) - Relay ON (PLTS)");
    } else {
      digitalWrite(RELAY_Inv, LOW);
      digitalWrite(RELAY_ATS_F, LOW);
      digitalWrite(RELAY_ATS_N, LOW);
