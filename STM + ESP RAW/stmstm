...
// Timer untuk Reset Energi PZEM (setiap 24 jam)
unsigned long lastEnergyResetTimestamp = 0;  // Timestamp terakhir reset energi
const unsigned long energyResetInterval = 86400;  // 24 jam dalam detik (86400)
...
void initializeCSVFiles() {
    ...
    if (!SD.exists("/LOAD.csv")) {
        ...
        if (file) {
            file.println("No;Timestamp;PLTS Power (W);PLTS Energy (Wh);Grid Power (W);Grid Energy (Wh);Lux (lx);Temp1 (C);Temp2 (C)");
            file.close();
...
// ===== FUNGSI RESET ENERGY COUNTER PZEM =====
bool resetPZEMEnergy(ModbusMaster &node, const char* name) {
    while (Serial2.available()) Serial2.read();
    delay(100);
    
    // Reset energy dengan menulis ke register 0x0042
...
void resetAllPZEMEnergy() {
    Serial.println("\n🔄 [RESET] Starting 24-hour energy reset for all PZEM...");
    
    // Reset PZEM Panel (PV)
    delay(200);
    resetPZEMEnergy(nodePanel, "PZEM Panel (PV)");
    
    // Reset PZEM Battery
    delay(200);
    resetPZEMEnergy(nodeBattery, "PZEM Battery");
    
    // Reset PZEM AC
    delay(200);
    resetPZEMEnergy(nodeAC, "PZEM AC");
    
    // Update timestamp terakhir reset
    lastEnergyResetTimestamp = getCurrentTimestamp();
    
    Serial.print("✅ [RESET] All energy counters reset complete at timestamp: ");
    Serial.println(lastEnergyResetTimestamp);
    Serial.println("⏰ [RESET] Next reset will occur in 24 hours\n");
}

void checkAndResetEnergy() {
    // Inisialisasi timestamp pertama kali
    if (!energyResetInitialized && timeRefAvailable) {
        lastEnergyResetTimestamp = getCurrentTimestamp();
        energyResetInitialized = true;
        Serial.println("🔧 [RESET] Energy reset timer initialized");
        return;
    }
    
    // Skip jika timeref belum tersedia
    if (!timeRefAvailable) {
        return;
    }
    
    unsigned long currentTimestamp = getCurrentTimestamp();
    unsigned long elapsedSeconds = currentTimestamp - lastEnergyResetTimestamp;
    
    // Cek apakah sudah 24 jam (86400 detik)
    if (elapsedSeconds >= energyResetInterval) {
        Serial.print("⏰ [RESET] 24 hours elapsed (");
    ...
    if (Serial3.available()) {
        ...
        if (commaCount == 12) {
            ...
            if (commaIndex == 13) {
                ...
                Serial.print("   ⏰ Timestamp: "); Serial.print(espTimestamp);
                if (timestampValid) {
                    Serial.println(" ✅ VALID");
                } else {
                    Serial.print(" ⚠️ INVALID (expected > 1000000, got: ");
                ...
                if (timestampValid && espTimestamp > globalTimeRef) {
                    if (!timeRefAvailable || (millis() - lastTimeRefUpdate) >= timeRefUpdateInterval) {
                        Serial.print("   🔄 Updating TimeRef: ");
                        Serial.print(globalTimeRef);
                        Serial.print(" -> ");
                        Serial.println(espTimestamp);
                        updateTimeRef(espTimestamp);
                    } else {
                        Serial.println("   ⏭ TimeRef skip (recently updated)");
                    }
                } else if (!timestampValid) {
                    Serial.println("   ❌ TimeRef NOT updated (invalid timestamp)");
                } else if (espTimestamp <= globalTimeRef) {
                    Serial.print("   ⏪ TimeRef NOT updated (old timestamp: ");
                    Serial.print(espTimestamp);
                    Serial.print(" <= ");
                    Serial.print(globalTimeRef);
                    Serial.println(")");
...
void ATS() {
  ...
  Serial.println("ℹ️  ATS: Manual control mode (SOC logic disabled)");
}
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (command == "sd" || command == "sdtest") {
            ...
            Serial.println("📡 === ESP32 DATA STATUS ===");
        ...
        } else if (command == "resetenergy" || command == "energyreset") {
            Serial.println("🔄 [MANUAL] Force energy reset...");
            resetAllPZEMEnergy();
        } else if (command == "energyinfo" || command == "energystatus") {
            Serial.println("📊 === ENERGY RESET STATUS ===");
            Serial.print("Initialized: "); Serial.println(energyResetInitialized ? "YES" : "NO");
            Serial.print("Last Reset Timestamp: "); Serial.println(lastEnergyResetTimestamp);
            Serial.print("Current Timestamp: "); Serial.println(getCurrentTimestamp());
            if (energyResetInitialized && timeRefAvailable) {
                unsigned long elapsed = getCurrentTimestamp() - lastEnergyResetTimestamp;
                Serial.print("Time Since Last Reset: "); Serial.print(elapsed); Serial.println(" seconds");
                Serial.print("Time Until Next Reset: "); Serial.print(energyResetInterval - elapsed); Serial.println(" seconds");
                Serial.print("Hours Until Next Reset: "); Serial.println((energyResetInterval - elapsed) / 3600.0, 1);
            } else {
                Serial.println("⚠️ Waiting for TimeRef initialization...");
            }
            Serial.println("--- CURRENT ENERGY VALUES ---");
            Serial.print("PV Energy: "); Serial.print(PZEMEnergyPanel, 0); Serial.println(" Wh");
            Serial.print("Battery Energy: "); Serial.print(PZEMEnergyBattery, 0); Serial.println(" Wh");
            Serial.print("PLTS Energy: "); Serial.print(PLTSEnergy, 0); Serial.println(" Wh");
            Serial.print("Grid Energy: "); Serial.print(GridEnergy, 0); Serial.println(" Wh");
        }
    }
